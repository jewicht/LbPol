...
void coeff2_rrv2double(RooRealVar* coeff2[ordermax2i][ordermax2j], double dcoeff2[ordermax2i][ordermax2j])
{
  for (int i=0; i<ordermax2i;i++) {
    for (int j=0; j<ordermax2j;j++) {
      if (coeff2[i][j]) {
	dcoeff2[i][j]=coeff2[i][j]->getVal();
      } else {
	dcoeff2[i][j]=0.;
      }
    }
...
void readcoeff2_rrv(const TString& filename, RooRealVar* coeff2[ordermax2i][ordermax2j])
{
  ...
  for (int i=0; i<ordermax2i;i++) {
    for (int j=0; j<ordermax2j;j++) {
      if (i+j>0 and coeff2[i][j]) {
	RooRealVar* var = (RooRealVar*)floatPars.find(coeff2[i][j]->GetName());
	if (var) {
	  coeff2[i][j]->setVal(var->getVal());
	  coeff2[i][j]->setError(var->getError());
	  std::cerr << "readcoeff2_rrv: could find " <<  coeff2[i][j]->GetName() << std::endl;
	} else {
	  std::cerr << "readcoeff2_rrv: can't find " <<  coeff2[i][j]->GetName() << std::endl;
	}
      }
    }
  }
  coeff2[0][0]->setVal(1.);
  coeff2[0][0]->setConstant();
  delete fr;
}
...
void createRRV2(bool doBd2JpsiKS, RooRealVar* coeff2[ordermax2i][ordermax2j],TString prefix, int maxorderi, int maxorderj, int maxtotorder, int maxcorrel, const TString& filename)
{
  const double dmin=-1.;
  const double dmax=1.;
  for (int i=0; i<ordermax2i;i++) {
	...
	if (
	    ...
	    (i+j<=8) and
	    (i+j<=maxtotorder)) {
	  coeff2[i][j] = new RooRealVar(prefix + "-c" + istr(i,"02") + istr(j,"02"), prefix + "-c" + istr(i,"02") + istr(j,"02"),0.,dmin,dmax);
	}
      } else {
	if (
	    //	    ((i%2==0) or (i==1)) and 
  ...
  coeff2[0][0]->setConstant();
  //  mygetchar;
  if (filename!="") readcoeff2_rrv(filename,coeff2);
}

void createRooLegendre2(RooRealVar& costheta, RooRealVar& costheta1, TString prefix, RooAbsPdf*& pdf, RooRealVar* coeff2[ordermax2i][ordermax2j])
{
  RooArgList varlist;
  for (int i=0; i<ordermax2i;i++) {
    for (int j=0; j<ordermax2j;j++) {
      if (coeff2[i][j]) varlist.add(*coeff2[i][j]);	  
    }
  }
  pdf = new RooLegendre2Pdf(prefix,prefix,costheta,costheta1,varlist);//, ordermax3i, ordermax3j, ordermax3k);
}
