...
RooRealVar mass("M","M(J/#psi#Lambda)",5500.,5750.,"MeV/#it{c}^{2}");
...
RooRealVar P_b_Lbbar("P_b_Lbbar","P_{b} (Lbbar)",0.0, -1.5, 1.5);//0.50
RooRealVar alpha_b("alpha_b","#alpha_{b}",1.0,-1.1,1.1);//0.457
	       ...
	       )
{
  ...
  if (doplot and dorand==0) {
    ...
    plot(c,RooArgList(mass), tot_mass_model, tot_data, plotname, plotpull, &masspdfs, "", &cat, plotlhcb); 
    c.SetLogy(0);
    mygetchar;
  }
...
//  std::cout << std::endl;
  ...
  for (int i=0; i<angpdflist.getSize(); i++) {
    ...
    if (doprefit) {
      ...
      if (doplot and dorand==0) plot(c,*obs, *sig_ang_model, tot_data_w, plotname, plotpull, NULL, "", &cat, plotlhcb);
      frtmp2->Print("v");
    ...
    if (!isconverged(fr)) exit(1);

    
    //fit probability
    RooDataHist rdh("rdh","rdh",RooArgList(costheta,costheta1,costheta2,cat), *tot_data_w);
    RooChi2Var chi2("chi2","chi2", *sig_ang_model,rdh);
    ...
    if (doplot and dorand==0) {
      ...
      plot(c,*obs, *sig_ang_model, tot_data_w, plotname, plotpull, NULL, "", &cat, plotlhcb);
    ...
    if (dolikscan) {
      ...
      if (useparam==0) {
      	...
      	am2=0.4;
      	bm2=0.;
	plotlikelihoodscans(plotname,c,ap2,-0.15,0.35,RooArgList(P_b,am2,bm2),tot_data_w,sig_ang_model,extconstangfit);

      	P_b=0.05;
	am2=0.4;
      	ap2=0.;
	plotlikelihoodscans(plotname,c,bm2,-0.15,0.5,RooArgList(P_b,ap2,am2),tot_data_w,sig_ang_model,extconstangfit);
...
int main(int argc, char** argv)
{
  ...
  ConfigFile cf("datafitconfig.txt");
...
//   return 1;
  ...
  if (useparam==0) {
    ...
    sig_ang_model = new RooLbtoJpsiL0PDF3("sig_ang_model","sig_ang_model",costheta,costheta1,costheta2,P_b, alpha_b_rfv, alpha_lambda, r_0_rfv, r_1_rfv);
    sig_ang_model_Lbbar = new RooLbtoJpsiL0PDF3("sig_ang_model_Lbbar","sig_ang_model_Lbbar",costheta,costheta1,costheta2,P_b, alpha_b_rfv, alpha_lambdabar, r_0_rfv, r_1_rfv);
  ...
  if (useparam==1)  {
	...
	for (int j=0; j<ordermax2j;j++) {
      ...
      sig_ang_model = new RooLbtoJpsiL0PDF3("sig_ang_model","sig_ang_model",costheta,costheta1,costheta2,P_b, alpha_b, alpha_lambda, r_0, r_1);
      sig_ang_model_Lbbar = new RooLbtoJpsiL0PDF3("sig_ang_model_Lbbar","sig_ang_model_Lbbar",costheta,costheta1,costheta2,P_b_Lbbar, alpha_b, alpha_lambda, r_0, r_1);
    }
  ...
  if (oper=="merge") {
    ...
    for (int i=1;i<dorand;i++) {
      ...
      if (fr_mass and fr_ang and checkweirdpulls(fr_ang, &vars)) {
    ...
    TString outputname("datafit-syst-");
  ...
  if (usekeyspdf) {
    ...
    if (randmccalib and dorand>0) {
      acceptancell = new calcacceptanceclass(acceptancefile + "-ll-toy" + istr(dorand%200,"06") + ".root");
      acceptancedd = new calcacceptanceclass(acceptancefile + "-dd-toy" + istr(dorand%200,"06") + ".root");
    } else {
  ...
  const TString datafilename(rootdir + "DVTuples_data_stripping17.reduced.root");
  ...
  RooArgList mass_dd_pdfs(sig_dd_mass_model,*bkg_dd_mass_model);
  if (separateLbLbar) mass_dd_pdfs.add(RooArgList(*bkg_dd_Lbbar_mass_model));
  RooArgList mass_ll_pdfs(sig_ll_mass_model,*bkg_ll_mass_model);
  if (separateLbLbar) mass_ll_pdfs.add(RooArgList(*bkg_ll_Lbbar_mass_model));
